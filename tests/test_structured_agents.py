...
class TestRenderTraderProposal:
    ...
    def test_optional_fields_included_when_present(self):
        ...
        assert "**Entry Price (建議入場價)**: 189.5" in md
        assert "**Target / Exit Price (建議離場價)**: 220.0" in md
        assert "**Stop Loss (止蝕價)**: 178.0" in md
...
class TestTraderAgent:
    def test_structured_path_produces_rendered_markdown(self):
        ...
        assert "**Entry Price (建議入場價)**: 189.5" in plan
...
class TestSentimentAnalystAgent:
    ...
    def _stub_sentiment_feeds(self, monkeypatch):
        """Keep prompts small so unit tests exercise the structured path."""
        from tradingagents.dataflows.config import get_config

        cfg = dict(get_config())
        cfg["sentiment_freetext_first"] = False
        monkeypatch.setattr(
    ...
    def test_freetext_first_skips_structured_binding(self):
        from tradingagents.dataflows.config import get_config

        plain = "**Overall Sentiment:** **Bullish** (Score: 7.0/10)\n**Confidence:** High\n\nFree text."
        llm = MagicMock()
        llm.invoke.return_value = MagicMock(content=plain)
        structured = MagicMock()
        llm.with_structured_output.return_value = structured
        ...
        cfg = dict(get_config())
        cfg["sentiment_freetext_first"] = True
        with patch(
            "tradingagents.agents.analysts.sentiment_analyst.get_config",
            return_value=cfg,
        ):
