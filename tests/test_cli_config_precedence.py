...
def test_build_run_config_merges_long_term_profile():
    sel = {**SELECTIONS, "investment_mode": "long_term", "investment_horizon": "1y"}
    cfg = m._build_run_config(sel, checkpoint=None)
    assert cfg["investment_mode"] == "long_term"
    assert cfg["reflection_holding_days"] == 252
    assert cfg["fundamentals_statement_freq"] == "annual"
