...
def test_investment_mode_env_overrides(monkeypatch):
    dc = _reload_with_env(
        monkeypatch,
        TRADINGAGENTS_INVESTMENT_MODE="long_term",
        TRADINGAGENTS_INVESTMENT_HORIZON="1y",
        TRADINGAGENTS_SCREENER_TOP_N="15",
    )
    assert dc.DEFAULT_CONFIG["investment_mode"] == "long_term"
    assert dc.DEFAULT_CONFIG["investment_horizon"] == "1y"
    assert dc.DEFAULT_CONFIG["screener_top_n"] == 15
