...
def select_investment_mode() -> str:
    """Select short-term trading vs long-term investing workflow."""
    choice = questionary.select(
        "Select Investment Mode:",
        choices=[
            questionary.Choice(
                "Short-term — tactical trading (default lookbacks, trading report)",
                "short_term",
            ),
            questionary.Choice(
                "Long-term — 6 months to 5+ years (extended lookbacks, investment memo)",
                "long_term",
            ),
        ],
        instruction="\n- Use arrow keys to navigate\n- Press Enter to select",
    ...
    if choice is None:
        console.print("\n[red]No investment mode selected. Exiting...[/red]")
        exit(1)
    return choice


def select_investment_horizon() -> str:
    """Select holding horizon for long-term mode."""
    choice = questionary.select(
        "Select Investment Horizon:",
        choices=[
            questionary.Choice("6 months", "6m"),
            questionary.Choice("1 year", "1y"),
            questionary.Choice("3 years (default)", "3y"),
