...
from cli.utils import (
    ...
    select_investment_horizon,
    select_investment_mode,
    select_llm_provider,
...
def update_display(layout, spinner_text=None, stats_handler=None, start_time=None):
    ...
    if message_buffer.investment_mode_label:
        stats_parts.append(message_buffer.investment_mode_label)
...
def get_user_selections(
    ...
    # Step 0: Investment mode (skipped when set via env or CLI flag)
    if investment_mode:
        selected_investment_mode = investment_mode
        console.print(
            f"[green]✓ Investment mode from CLI:[/green] {selected_investment_mode}"
        )
    elif os.environ.get("TRADINGAGENTS_INVESTMENT_MODE"):
        selected_investment_mode = DEFAULT_CONFIG["investment_mode"]
        console.print(
            f"[green]✓ Investment mode from environment:[/green] {selected_investment_mode}"
        )
    else:
        console.print(
            create_question_box(
                "Step 0: Investment Mode",
                "Short-term trading vs long-term investing (6m–5y+)",
            )
        )
        selected_investment_mode = select_investment_mode()

    if selected_investment_mode == "long_term":
        if investment_horizon:
            selected_horizon = investment_horizon
            console.print(f"[green]✓ Horizon from CLI:[/green] {selected_horizon}")
        elif os.environ.get("TRADINGAGENTS_INVESTMENT_HORIZON"):
            selected_horizon = DEFAULT_CONFIG["investment_horizon"]
            console.print(
                f"[green]✓ Horizon from environment:[/green] {selected_horizon}"
            )
        else:
            console.print(
                create_question_box(
                    "Step 0b: Investment Horizon",
                    "Target holding period for long-term analysis",
                )
            )
            selected_horizon = select_investment_horizon()
    else:
        selected_horizon = DEFAULT_CONFIG.get("investment_horizon", "3y")
    ...
    return {
        "investment_mode": selected_investment_mode,
...
def _build_run_config(selections: dict, checkpoint: bool | None) -> dict:
    ...
    config["investment_mode"] = selections.get("investment_mode", "short_term")
    config["investment_horizon"] = selections.get("investment_horizon", "3y")
    return merge_mode_into_config(config)
...
def run_analysis(
    ...
    message_buffer.investment_mode_label = mode_label(config)
    ...
    with Live(layout, refresh_per_second=4):
        ...
        message_buffer.add_message(
            "System", f"Investment mode: {message_buffer.investment_mode_label}"
        )
        message_buffer.add_message(
            "System",
...
def analyze(
    ...
    mode: str | None = typer.Option(
        None,
        "--mode",
        help="Investment mode: short_term (default) or long_term.",
    ),
    horizon: str | None = typer.Option(
        None,
        "--horizon",
        help="Holding horizon when --mode long_term: 6m, 1y, 3y, or 5y+.",
    ),
):
    ...
    if mode and mode not in ("short_term", "long_term"):
        console.print("[red]--mode must be short_term or long_term[/red]")
        raise typer.Exit(1)
    if horizon and horizon not in ("6m", "1y", "3y", "5y+"):
        console.print("[red]--horizon must be 6m, 1y, 3y, or 5y+[/red]")
        raise typer.Exit(1)
    run_analysis(checkpoint=checkpoint, investment_mode=mode, investment_horizon=horizon)
...
def screen(
    universe: str = typer.Option(
        ...
        "--universe",
        "-u",
        help="Stock universe: sp500 or nasdaq100.",
    ),
    universe_file: str | None = typer.Option(
        None,
        "--universe-file",
        help="Path to a custom ticker list (one symbol per line).",
    ),
    horizon: str = typer.Option(
        "3y",
        "--horizon",
        help="Investment horizon for scoring context: 6m, 1y, 3y, or 5y+.",
    ),
    top: int = typer.Option(
        20,
        "--top",
        "-n",
        help="Number of top-ranked tickers to keep in the watchlist.",
    ),
    analyze_top: int = typer.Option(
        0,
        "--analyze-top",
        help="After screening, run deep long-term analysis on the top N tickers "
        "(requires LLM configured via env or .env).",
    ),
    output_dir: str | None = typer.Option(
        None,
        "--output-dir",
        help="Directory for watchlist output (default: results/screening/<date>_<universe>_<horizon>).",
    ),
):
    """Quantitative long-term stock screen (yfinance fundamentals)."""
    from tradingagents.dataflows.config import set_config
    from tradingagents.screening.runner import run_screen

    if horizon not in ("6m", "1y", "3y", "5y+"):
        console.print("[red]--horizon must be 6m, 1y, 3y, or 5y+[/red]")
        raise typer.Exit(1)

    stamp = datetime.datetime.now().strftime("%Y-%m-%d")
    uni_label = Path(universe_file).stem if universe_file else universe
    out = (
        Path(output_dir)
        if output_dir
        else Path(DEFAULT_CONFIG["results_dir"])
        / "screening"
        / f"{stamp}_{uni_label}_{horizon}"
    )

    screen_config = merge_mode_into_config({
        **DEFAULT_CONFIG,
    ...
    set_config(screen_config)

    console.print(
        f"[cyan]Screening[/cyan] universe={uni_label} horizon={horizon} top={top} ..."
    )
    result = run_screen(
        universe=universe,
        top_n=top,
    ...
    console.print(
        f"[green]✓ Scored {result['total_scored']} tickers; "
        f"watchlist saved to {out}[/green]"
    )

    if analyze_top <= 0:
        return

    watchlist = result.get("watchlist", [])[:analyze_top]
    if not watchlist:
        console.print("[yellow]No tickers to analyze.[/yellow]")
        return

    analysis_date = datetime.datetime.now().strftime("%Y-%m-%d")
    analysts = ["market", "social", "news", "fundamentals"]
    console.print(
        f"[cyan]Running deep analysis on top {len(watchlist)} tickers "
        f"(LLM from env / DEFAULT_CONFIG)...[/cyan]"
    )
    ...
    for row in watchlist:
        ...
        try:
            graph = TradingAgentsGraph(analysts, config=screen_config, debug=False)
            ...
            for chunk in graph.graph.stream(init_state, **args):
                final_state.update(chunk)
            save_path = out / "analyses" / ticker
            write_report_tree(final_state, ticker, save_path)
            console.print(f"[green]✓ Reports saved to {save_path}[/green]")
